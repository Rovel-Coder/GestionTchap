...
async function start() {
  ...
  try {
    ...
    // One Time Keys corrompues : purger le store crypto et redémarrer proprement
    if (e.message && e.message.includes('One time key') && e.message.includes('already exists')) {
      console.warn('[E2EE] Clés OTP corrompues détectées — purge du store crypto et redémarrage...');
      matrixClient.stop();
      matrixClient = null;
      _ready = false;

      const cryptoDb  = path.join(DATA_DIR, 'crypto.db');
      const sessionJson = path.join(DATA_DIR, 'session.json');
      ...
      console.log('[E2EE] Store purgé — redémarrage dans 2s...');
      await new Promise(r => setTimeout(r, 2000));
      process.exit(0); // Docker restart: unless-stopped relance proprement
    }
